...
int main(int argc, char *argv[])
{
	...
	if(semid == -1)
	{
		perror("Semaphores connection error - process 1");
		exit(1);
	}
	...
	if(shmid == -1)
	{
		perror("Shared memory connection error - process 1");
		exit(1);
	}
	...
	if(bufor == NULL)
	{
		perror("Linking shared memory error - process 1");
		exit(1);
	}

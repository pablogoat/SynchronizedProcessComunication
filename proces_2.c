...
int main(int argc, char *argv[])
{
	//parent = atoi(argv[1]);
	parent = getppid();
	int semid = semget(45288, 4, 0);
	if(semid == -1)
	{
		perror("Semaphores connection error - process 2");
		exit(1);
	}
	...
	if(shmid == -1)
	{
		perror("Shared memory connection error - process 2");
		exit(1);
	}
	...
	if(bufor == NULL)
	{
		perror("Linking shared memory error - process 2");
		exit(1);
	}
	...
	while(run)
	{
		if(work){
			...
			strcat(cos, "-2");
			printf("P2- converted: %s\n", cos);
			
			memcpy(bufor, cos, sizeof(cos));
			setSem(semid, 2, 1);
			sleep(1);
		}
